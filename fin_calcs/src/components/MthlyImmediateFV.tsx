...
import { mthlyAnnuityImmediateFV, iMthly } from "../formulas/0-compound_interest"
import React from "react"

const MthlyImmediateFV = () => {
    ...
    const calculateMthlyAnnImmPV = () => {
        ...
        const mthlyAnnImmPV = mthlyAnnuityImmediateFV(i, m, n)
        const im = iMthly(i, m)
        ...
        setM(0)
        setIM(im)
        setMessage(`The m-thly annuity immediate FV factor at rate ${i} pa, effective, convertible ${12/m} time(s) pa and period ${n}`)
        setSubmitted(false)
    }
...
export default MthlyImmediateFV